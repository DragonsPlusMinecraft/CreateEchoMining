package plus.dragons.createechomining.contraptions.mining.portabledrill;
...
import plus.dragons.createechomining.entry.CemEntityTypes;
...
public class PortableDrillEntity extends Entity {
    ...
    public PortableDrillEntity(Level Level, double X, double Y, double Z, int durability) {
        this(CemEntityTypes.PORTABLE_DRILL.get(), Level);
    ...
    public void tick() {
        ...
        if(isOnGround()){
            ...
            if(!level.getFluidState(blockPos).isEmpty())
                collapse();
            var others = level.getEntities(CemEntityTypes.PORTABLE_DRILL.get(), this.getBoundingBox(), entity->true);
            others.remove(this);
            if(!others.isEmpty()){
    ...
    public InteractionResult interact(Player pPlayer, @NotNull InteractionHand pHand) {
        ...
        if(AllItems.WRENCH.isIn(item)){
            if(!level.isClientSide()){
                ...
                var drop = CemItems.PORTABLE_DRILL.asStack();
